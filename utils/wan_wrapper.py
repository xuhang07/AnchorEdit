...
class WanTextEncoder(torch.nn.Module):
    def __init__(self, wan_models_dir) -> None:
        super().__init__()
        ...
        self.text_encoder.load_state_dict(
            torch.load(f"{wan_models_dir}/models_t5_umt5-xxl-enc-bf16.pth",
                       map_location='cpu', weights_only=False)
        )

        self.tokenizer = HuggingfaceTokenizer(
            name=f"{wan_models_dir}/google/umt5-xxl/", seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self, wan_models_dir):
        super().__init__()
        mean = [
        ...
        self.model = _video_vae(
            pretrained_path=f"{wan_models_dir}/Wan2.1_VAE.pth",
            z_dim=16,
        ).eval().requires_grad_(False)
    ...
    def __init__(
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                f"{model_name}", model_type=model_type, local_attn_size=local_attn_size, sink_size=sink_size,independent_first_frame=independent_first_frame,num_frame_per_block=num_frame_per_block,cross_attn_current_prompt_only=cross_attn_current_prompt_only,temporal_rope_stride=temporal_rope_stride,bidirectional_self_attn=bidirectional_self_attn)
        else:
