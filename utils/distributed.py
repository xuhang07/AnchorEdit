...
def fsdp_wrap(module, sharding_strategy="full", mixed_precision=False, wrap_strategy="size", min_num_params=int(5e7), transformer_module=None, cpu_offload=False, sync_module_states=False):
    ...
    module = FSDP(
        ...
        # 默认 False（保持原行为：所有 rank 都用各自加载的 state_dict）。
        # 设为 True 后：仅 rank0 持有正确权重，FSDP 在 wrap 时会自动从 rank0
        # 把权重 broadcast 到其它 rank，可避免每个 rank 都在 CPU 上加载整份
        # ckpt 而 OOM（exitCode 137 / OOMKilled）。
