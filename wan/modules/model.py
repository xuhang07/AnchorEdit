...
# @amp.autocast(enabled=False)
def rope_apply(x, grid_sizes, freqs, temporal_rope_stride=1):
    ...
    for i, (f, h, w) in enumerate(grid_sizes.tolist()):
        ...
        x_i = torch.view_as_complex(x[i, :seq_len].to(torch.float64).reshape(
            seq_len, n, -1, 2))
        # temporal indices: 0, stride, 2*stride, ... for temporal_rope_stride > 1
        t_indices = torch.arange(f, device=freqs[0].device) * temporal_rope_stride
        freqs_i = torch.cat([
...
class WanT2VCrossAttention(WanSelfAttention):
    def forward(self, x, context, context_lens, cross_attn_mask=None, crossattn_cache=None, **kwargs):
        r"""
        Args:
            x(Tensor): Shape [B, L1, C] (Video frames)
            context(Tensor): Shape [B, L2, C] (Text prompts)
            context_lens(Tensor): Shape [B]
            cross_attn_mask: BlockMask for teacher-forcing cross attention
            crossattn_cache (List[dict], *optional*): Cached key and value tensors.
        """
        b, s, n, d = *x.shape[:2], self.num_heads, self.head_dim
        s_kv = context.size(1)
        ...
        # 2. 生成 Key, Value (带 Cache 逻辑)
        if crossattn_cache is not None:
            if not crossattn_cache["is_init"]:
                crossattn_cache["is_init"] = True
                k = self.norm_k(self.k(context)).view(b, s_kv, n, d)
                v = self.v(context).view(b, s_kv, n, d)
                crossattn_cache["k"] = k
                crossattn_cache["v"] = v
            else:
                k = crossattn_cache["k"]
        ...
        if cross_attn_mask is not None:
            # --- Padding 逻辑以适配 flex_attention ---
            # Q 侧 Padding (视频帧长度 s)
            padded_q_len = math.ceil(s / 128) * 128 - s
            if padded_q_len > 0:
                padded_q = torch.cat([
                    q, 
                    torch.zeros([b, padded_q_len, n, d], device=q.device, dtype=q.dtype)
                ], dim=1)
            else:
                padded_q = q

            # KV 侧 Padding (文本长度 s_kv)
            padded_kv_len = math.ceil(s_kv / 128) * 128 - s_kv
            if padded_kv_len > 0:
                padded_k = torch.cat([
                    k, 
                    torch.zeros([b, padded_kv_len, n, d], device=k.device, dtype=k.dtype)
                ], dim=1)
                padded_v = torch.cat([
                    v, 
                    torch.zeros([b, padded_kv_len, n, d], device=v.device, dtype=v.dtype)
                ], dim=1)
            else:
                padded_k, padded_v = k, v

            # 调用 flex_attention
            # 注意: block_mask 的 Q_LEN 和 KV_LEN 必须与 padded 后的长度一致
            res = flex_attention(
                query=padded_q.transpose(1, 2), # [B, N, S_pad, D]
                key=padded_k.transpose(1, 2),
                value=padded_v.transpose(1, 2),
                block_mask=cross_attn_mask
            )
            
            # 切回原始 Q 长度并转置回 [B, S, N, D]
            out = res[:, :, :s].transpose(1, 2)
        else:
            # 推理模式或标准 FlashAttention
...
class WanI2VCrossAttention(WanSelfAttention):
    ...
    def forward(self, x, context, context_lens, cross_attn_mask=None, **kwargs):
        r"""
        Args:
            x(Tensor): Shape [B, L1, C] (Video frames)
            context(Tensor): Shape [B, L2, C] (257 CLIP + N*512 Text)
            context_lens: Text lengths
            crossattn_mask: BlockMask for teacher-forcing cross attention
        """
        # 1. 拆分 CLIP 图像特征和文本特征
        ...
        b, s, n, d = *x.shape[:2], self.num_heads, self.head_dim
        s_kv = context_text.size(1)

        # 2. 生成 Q, K, V
        q = self.norm_q(self.q(x)).view(b, s, n, d)
        
        # Image KV (CLIP) - 全局可见，不使用 Mask
        k_img = self.norm_k_img(self.k_img(context_img)).view(b, -1, n, d)
        v_img = self.v_img(context_img).view(b, -1, n, d)
        img_x = flash_attention(q, k_img, v_img, k_lens=None)

        # Text KV (Prompts)
        k_text = self.norm_k(self.k(context_text)).view(b, s_kv, n, d)
        v_text = self.v(context_text).view(b, s_kv, n, d)

        # 3. 计算 Text Attention
        if cross_attn_mask is not None:
            # --- 参考 self-attn 的 Padding 风格 ---
            # Q 侧 Padding (视频帧)
            padded_q_len = math.ceil(s / 128) * 128 - s
            padded_q = torch.cat([
                q, 
            ...
            # KV 侧 Padding (文本 Prompt)
            padded_kv_len = math.ceil(s_kv / 128) * 128 - s_kv
            padded_k = torch.cat([
                k_text, 
                torch.zeros([b, padded_kv_len, n, d], device=k_text.device, dtype=k_text.dtype)
            ], dim=1)
            padded_v = torch.cat([
                v_text, 
                torch.zeros([b, padded_kv_len, n, d], device=v_text.device, dtype=v_text.dtype)
            ], dim=1)

            # flex_attention 调用
            # 注意: block_mask 的 Q_LEN 和 KV_LEN 必须与这里的 padded 长度一致
            res = flex_attention(
                query=padded_q.transpose(1, 2),
                key=padded_k.transpose(1, 2),
                value=padded_v.transpose(1, 2),
                block_mask=cross_attn_mask
            )
            
            # 切回原始 Q 长度并转置 [B, N, S, D] -> [B, S, N, D]
            x_text = res[:, :, :s].transpose(1, 2)
        else:
            # 推理或非 TF 模式
            x_text = flash_attention(q, k_text, v_text, k_lens=context_lens)

        # 4. 合并并输出
        # 将 [B, S, N, D] 展平为 [B, S, C]
        x = x_text.reshape(b, s, -1) + img_x.reshape(b, s, -1)
        x = self.o(x)
        return x
